...
def listar_equipes():
    ...
    for equipe in equipes:
        ...
        result.append({
            ...
            'status': equipe.status.value if equipe.status else None,
            'membros': membros
        })
...
def listar_equipes_ativas():
    ...
    return jsonify(result)
@equipe_bp.route('/equipes/ativas/<int:id>', methods=['GET'])
...
def desativar_equipe(id):
    equipe = EquipeTecnica.query.get_or_404(id)
    url_ordens = f'https://mode-mls-married-alphabetical.trycloudflare.com/orders/filter?teamId={id}'
    try:
